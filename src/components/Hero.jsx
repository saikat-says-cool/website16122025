...
const Hero = () => {
    ...
    return (
        <div className="min-h-screen pt-32 pb-16 px-6 md:px-12 max-w-7xl mx-auto flex flex-col justify-center animate-fade-in">
            <div className="inline-block border border-black px-3 py-1 text-[10px] md:text-xs font-tech uppercase tracking-widest mb-6 w-max">
                Target: MSSPs • Pentesting • Compliance
            </div>

            <h1 className="text-4xl sm:text-5xl md:text-7xl leading-[1.1] mb-8 max-w-4xl">
                Predictable Revenue <span className="highlight-scribble text-[#2B4C7E]">Infrastructure</span> for Cybersecurity Firms.
            </h1>

            <p className="text-lg md:text-2xl font-hand italic text-gray-700 max-w-2xl mb-10 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-20">
                <button onClick={() => navigate('/blueprint')} className="btn-primary w-full sm:w-auto text-center">
                    See The Blueprint
                </button>
                <button onClick={() => navigate('/casestudy')} className="px-6 py-3 font-tech text-xs md:text-sm border-b border-black hover:bg-gray-200 transition-colors text-left sm:text-center md:text-left">
                    Read Case Study: Project Texas Health &rarr;
                </button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 border-t border-gray-300 pt-8">
                <div>
